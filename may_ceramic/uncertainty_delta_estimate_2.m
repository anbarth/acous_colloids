...
y_init = zipParamsCrossoverFudge(eta0_orig, phi0_orig, myDelta, A_orig, width_orig, sigmastar_orig, C_orig, phi_fudge_orig);
resnorm_plus =  sum(residualsCrossoverFudge(dataTable, phi_list, volt_list, y_init));
showCollapse(dataTable,y_optimal);
title('+epsilon')
...
myDelta = delta_center-epsilon;
y_init = zipParamsCrossoverFudge(eta0_orig, phi0_orig, myDelta, A_orig, width_orig, sigmastar_orig, C_orig, phi_fudge_orig);
resnorm_minus =  sum(residualsCrossoverFudge(dataTable, phi_list, volt_list, y_init));
showCollapse(dataTable,y_optimal);
title('-epsilon')
