...
sigmastar_avg = zeros(size(volt_list));
sigmastar_std = zeros(size(volt_list));
for jj=1:length(volt_list)
    volt = volt_list(jj);
    sigmastars = sigmastar_data(V_data==volt);
    sigmastar_avg(jj) = mean(sigmastars);
    sigmastar_std(jj) = std(sigmastars);
end

errorbar(volt_list,sigmastar_avg,sigmastar_std,'ok','LineWidth',1)
pSigmastar = polyfit(volt_list,sigmastar_avg,2);
%pSigmastar = polyfit(V_data,sigmastar_data,2);
V = linspace(0,80);
if makePlot
    plot(V,pSigmastar(1)*V.^2+pSigmastar(2)*V+pSigmastar(3),'-r','LineWidth',0.75);
end
