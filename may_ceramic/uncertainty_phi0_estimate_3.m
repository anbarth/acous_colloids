...
deltaPhi0 = 0.01;
phi0Range = linspace(phi0_init-deltaPhi0,phi0_init+deltaPhi0,10);
SSR_0 = costfxn(y_optimal);
SSR = zeros(size(phi0Range));
epsilon = zeros(size(phi0Range));
y_list = zeros(length(phi0Range),length(y_optimal));
for ii = 1:length(phi0Range)
    myphi0 = phi0Range(ii);
    my_y_init = setParams(y_optimal,13,'phi0',myphi0);
    my_lb = setParams(lower_bounds,13,'phi0',myphi0);
    my_ub = setParams(upper_bounds,13,'phi0',myphi0);  

    my_y_optimal = fmincon(costfxn,my_y_init,[],[],[],[],my_lb,my_ub,[],opts);
    mySSR = costfxn(my_y_optimal);

    epsilon(ii) = myphi0-phi0_init;
    SSR(ii) = mySSR-SSR_0;
