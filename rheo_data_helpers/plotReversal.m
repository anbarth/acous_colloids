function plotReversal(rheoData,color)
% 0. set up fig
%figure; hold on;
ax1 = gca;
%ax1.XScale = 'log';
...
% 2. get just the data with sigma = -0.3 (after reversal)
stress = getStress(rheoData,1); % (rheometer units)
strain = strain(stress == -0.3);
...
if nargin < 2
    plot(delta_strain,eta);
else
    plot(delta_strain,eta,'Color',color,'LineWidth',1.5);
end
