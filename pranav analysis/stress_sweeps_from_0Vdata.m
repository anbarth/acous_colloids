...
phisToInclude = [0.2,0.25,0.3,0.35,0.4,0.44,0.46,0.48,0.5,0.52,.53,.54,.55];
...
upper_bounds = [Inf,Inf,.75,.75];
lower_bounds = [0,0,0.551,.57];


opts = optimoptions('fmincon','Display','final','StepTolerance',1e-12);
%opts = optimoptions('fmincon','Display','off');
s = fmincon(costfxn, [2e-6, 1.3, 0.58, 0.64],constraintMatrix,constraintVector,...
            [],[],lower_bounds,upper_bounds,[],opts);

disp(s);
etaFit = fitfxn(s);
etaSilly = fitfxn([5e-6, 10.1811, 0.551, 0.57]);
