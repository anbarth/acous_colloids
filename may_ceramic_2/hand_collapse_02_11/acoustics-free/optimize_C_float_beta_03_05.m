...
y_init = [y_init 2]; % set beta=2 initially
log_y_init = log(abs(y_init));
myModelHandle = @modelHandpickedAllExp0VBeta;
...
disp(costfxn(log_y_fminsearch_0V))
disp(costfxn(log_y_lsq_0V))
disp(costfxn(log_y_fmincon_0V))
...
jacobian = numeric_jacobian(acoustics_free_data,y_fmincon_0V,myModelHandle);
...
disp(y_fminsearch_0V(end))
disp(confInts(end))

return


D_fminsearch = y_fminsearch_0V(7:end-1);
D_lsq = y_lsq_0V(7:end-1);
D_fmincon = y_fmincon_0V(7:end-1);
