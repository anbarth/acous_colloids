play_with_sigmastar_07_08;
y_init = y_handpicked_07_08;
log_y_init = log(abs(y_init));
myModelHandle = @modelLinearSigmastarA;
...
return
%% SHOW CONF INTS
ci = get_conf_ints(dataTable,y_lsq,@modelLinearSigmastarA);
s = ["F0","phi0","delta","A","h","sigma*_0","c"];
for ii=1:length(s)
    disp(s(ii))
    if ii==6
...
phiRange = 13:-1:1;
showLines = false;
show_F_vs_x(dataTable,y_fmincon,myModelHandle,'PhiRange',phiRange,'ShowLines',showLines,'ShowInterpolatingFunction',false,'ShowErrorBars',false); prettyplot
show_F_vs_xc_x(dataTable,y_fmincon,myModelHandle,'PhiRange',phiRange,'ShowLines',showLines,'ShowInterpolatingFunction',false,'ShowErrorBars',false); prettyplot
