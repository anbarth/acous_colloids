...
% fit sigma*_acous(E) to a line
linfit = fittype("a*x");
sigmastarFitParam = fit(E_list_restricted',CSS*sigmastar_acous',linfit,'StartPoint',1,'Weights',1./sigmastar_err);

sigmastarFit = @(V) CSS*sigmastar0V + sigmastarFitParam.a*acoustic_energy_density(V);

if makeSigmastarPlot
    figure; hold on;
    xlabel("Acoustic energy density {\itE_0} (Pa)")
    ylabel("\sigma^* (Pa)")


    % quadratic interpolation
    V = linspace(0,80);
    plot(acoustic_energy_density(V),sigmastarFit(V),'r-');
    ...
    for jj=1:length(volt_list_restricted)
        ...
        plot(acoustic_energy_density(volt_list_restricted(jj)),CSS*sigmastar(jj),'p','Color',colorV,'MarkerFaceColor',colorV,'MarkerSize',5,'LineWidth',1.5);
        prettyPlot;
        %xlim([0 100])
    end
    ...
    for volt=[5 20 80]
        ...
        plot(acoustic_energy_density(volt),sigmastarFit(volt),'p','MarkerFaceColor',colorV,'MarkerSize',10,'MarkerEdgeColor','r');
    end
