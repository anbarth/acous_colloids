...
ax_eta.YLabel.String = '\Delta\eta/\eta_0';
hold(ax_eta,'on');
cmap = winter(256);
...
for ii=1:length(sigma_list)
    ...
    eta0 = eta(V==0);
    deltaEta = eta-eta0;
    percentDeltaEta = deltaEta/eta0;
    %percent_eta = eta ./ eta(1);
    
    %plot(ax_eta,V,percent_eta, markerCode,'Color',myColor,'MarkerFaceColor',myColor,'LineWidth',1);
    plot(ax_eta,V,percentDeltaEta, markerCode,'Color',myColor,'MarkerFaceColor',myColor,'LineWidth',1);
