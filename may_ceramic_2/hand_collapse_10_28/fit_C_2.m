...
ft1 = fittype('(c-x)/(1+(x/a)^b)');
opts1 = fitoptions(ft1);
opts1.StartPoint = [0.4,-10,0.7];
...
%for jj=1:size(C,2)
    ...
    %p = polyfit(myPhi,myC,4);
    %plot(phiFake,p(1)*phiFake.^3+p(2)*phiFake.^2+p(3)*phiFake+p(4),'Color',myColor);
    %plot(phiFake,p(1)*phiFake.^4+p(2)*phiFake.^3+p(3)*phiFake.^2+p(4)*phiFake+p(5),'Color',myColor);

    myFit1 = fit(myPhi,myC,ft1,opts1);
    plot(phiFake,(myFit1.c-phiFake)./(1+(phiFake/myFit1.a).^myFit1.b),'Color',myColor,'LineWidth',1)
