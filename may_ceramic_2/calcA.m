...
phi0 = 0.7011;
sigmastar = 0.3011*ones(1,7);
%phi_fudge = zeros(size(phi_list))';
C = [0.01 0.025 0.05 0.1 0.25 0.5 0.75 0.8 0.85 0.925 0.95 0.975 1]*1/1.0162;
xc=1;
f = @(sigma) exp(-sigmastar(1) ./ sigma);
% parameters from interpolating_fit_0V_all_phi
eta0 = 0.027;
A = 0.0227;
delta = -1.1995;
width = 1.0162;
...
phi_range = 6:13; % which volume fractions to include
...
for ii = phi_range
    ...
    P = zeros(size(x));
    for kk = 1:length(x)
        P(kk) = getP(phi,sigma(kk),voltage(kk),stressTable);
    end
    ...
    for jj=1:length(x)
        myX = x(jj);
        myF = F(jj);
        % note that we CAN get a (aphysical) negative value for myX_target
        % this happens for small myF
...
P_all_old = P_all;
A_all_old = A_all;
keep_me_too = P_all~=0 & 1-abs(A_all)>0.0001;
