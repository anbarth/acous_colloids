...
[eta0, phi0, delta, A, width, sigmastar, C, phi_fudge] = unzipParamsHandpickedAll(y_in,numPhi);
y_out=y_in;
for ii=1:2:length(varargin)
    if isa(varargin{ii},'char')
        ...
        if strcmp(fieldName,'eta0')
            eta0 = varargin{ii+1};
            y_out = zipParamsHandpickedAll(eta0, phi0, delta, A, width, sigmastar, C, phi_fudge);
