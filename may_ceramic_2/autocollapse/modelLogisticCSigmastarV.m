function [x,F,delta_F,F_hat,eta,delta_eta,eta_hat] = modelLogisticCSigmastarV(stressTable, y)
...
sigmastar = @(V) sigmastarParams(1)*V^2 + sigmastarParams(2)*V + sigmastarParams(3);
f = @(sigma,V) exp(-sigmastar(V)./sigma);
C = @(phi,V) L./(1+exp(-k*(phi-phistar)));
...
for kk = 1:N
    ...
    D = (phi0-phi)^(-alpha) * C(phi,voltage);
    x(kk) = D*f(sigma,voltage);
...
F_hat = 1./x.^(2/alpha_interpolate) .* H_hat;
