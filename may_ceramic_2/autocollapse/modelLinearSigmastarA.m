function [x,F,delta_F,F_hat,eta,delta_eta,eta_hat] = modelLinearSigmastarA(stressTable, y)
...
U_list = acoustic_energy_density(volt_list);
CSS = (50/19)^3;
...
for kk = 1:N
    ...
    sigmastar = sigmastar0*CSS+c*U_list(jj);

    x(kk) = D(ii)*f(sigma,sigmastar);
