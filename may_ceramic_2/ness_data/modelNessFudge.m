function [x,F,delta_F,F_hat,eta,delta_eta,eta_hat] = modelNessFudge(stressTable, paramsVector)
...
D = paramsVector(7:7+numPhi-1);
phi_fudge = paramsVector(7+numPhi:end);
...
for kk = 1:N
    ...
    phi_eff = phi+my_phi_fudge;

    sigma = stressTable(kk,2);
    eta(kk) = stressTable(kk,4);        
    %delta_eta_rheo = max(stressTable(kk,5),eta(kk)*0.05);
    %delta_phi = 0.01;
    %delta_eta_volumefraction = eta(kk)*2*(phi0-phi_eff)^(-1)*delta_phi;
    %delta_eta(kk) = sqrt(delta_eta_rheo.^2+delta_eta_volumefraction.^2);

    x(kk) = D(ii)*f(sigma);
    F(kk) = eta(kk)*(phi0-phi_eff)^2;
    %delta_F(kk) = F(kk) .* (eta(kk).^(-2).*delta_eta_rheo.^2 + 4/(phi0-phi_eff)^2*delta_phi^2 ).^(1/2);
end
...
for kk = 1:N
    ...
    eta_hat(kk) = F_hat(kk)*(phi0-phi_eff)^-2;
end 
