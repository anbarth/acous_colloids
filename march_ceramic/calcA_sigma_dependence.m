...
fitfxn = @(k) exp(-((k(1)+k(3)*sigma_all).*P_all).^(k(2)));
costfxn = @(k) sum(( (fitfxn(k)-A_all) ).^2); 
%myK = fmincon(costfxn,[0.005,0.75],[0,0;0,0],[0,0]);
myK = fmincon(costfxn,[0.005,0.75,0],zeros(3),[0,0,0]);

P_fake = logspace(-4,6); 
%A_fake = exp(-(myK(1)*P_fake).^(myK(2)));
 %plot(ax_A,P_fake,A_fake,'k','LineWidth',1)
 %plot(ax_bulbul,P_fake,-1*log(A_fake),'k','LineWidth',1);
my_sigmas = [min(sigma_all), sigma_all(round(length(sigma_all)/2)), max(sigma_all)];
for ii=1:length(my_sigmas)
    my_sigma = my_sigmas(ii);
    A_fake = exp(-((myK(1)+myK(3)*my_sigma)*P_fake).^(myK(2)));
    myColor = log(my_sigma)*ones(size(P_fake));
    scatter(ax_A,P_fake,A_fake,[],myColor,'.');
    scatter(ax_bulbul,P_fake,-1*log(A_fake),[],myColor,'.');
end
