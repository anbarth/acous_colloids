...
for ii=1:size(C_init,1)
    C_init(ii,:) = C_init(ii,:)/(phi0_init-phi_list(ii));
end
...
% 0 < sigmastar < Inf
% 0 < C < Inf
% C = 1 for phi=20%, 30%; V > 0 (no data)
% C = 1 for phi=44%, V = 80 (not enough data)
C_lower = zeros(numPhi,numV);
C_upper = Inf*ones(numPhi,numV);
C_lower(1,2:end) = ones(size(C_lower(1,2:end)));
C_upper(1,2:end) = ones(size(C_upper(1,2:end)));
C_lower(2,2:end) = ones(size(C_lower(2,2:end)));
C_upper(2,2:end) = ones(size(C_upper(2,2:end)));
C_lower(3,7) = 1;
C_upper(3,7) = 1;
C_lower(3,5) = 1;
C_upper(3,5) = 1;
