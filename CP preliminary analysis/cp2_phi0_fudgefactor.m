...
% phi0 = 0.64;
% fitfxn = @(m) m*(phi0-phi_list);
% costfxn = @(x) sum(( (fitfxn(x)-eta12_list)./eta12_list ).^2);  
% myM = fmincon(costfxn,3,0,0);
% p = [-myM,myM*phi0];

% shift points left and right onto best fit line
phi_perfect = (eta12_list-p(2))/p(1);
delta_phi = phi_perfect-phi_list;


disp(-p(2)/p(1))
disp([phi_list;phi_perfect;delta_phi]')
figure; hold on;
scatter(phi_list,eta_list.^(-1/2))
scatter(phi_perfect,eta_list.^(-1/2))
