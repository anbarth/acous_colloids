...
airgap_depth = lambda_PLA/2; % [m]
airgap_width = lambda_PLA*1.5;

airgap_top = al_disk_top;
airgap_bot = al_disk_top+airgap_depth;
airgap_x_indices = kgrid.x_vec>airgap_top & kgrid.x_vec<airgap_bot;

left_edge = kgrid.y_vec(1);
right_edge = kgrid.y_vec(end);
elements_edges = left_edge:airgap_width:right_edge;
N_elements = length(elements_edges)-1;
for ii=1:N_elements
    if mod(ii,2)==0 %&& mod(ii,12)<7
        airgap_y_indices = kgrid.y_vec > elements_edges(ii) & kgrid.y_vec < elements_edges(ii+1);
        medium.sound_speed(airgap_x_indices, airgap_y_indices) = c_PLA;
        medium.density(airgap_x_indices, airgap_y_indices) = rho_PLA;
    end
end
...
sensor_height = al_disk_top - 0.8696e-3; % [m]
